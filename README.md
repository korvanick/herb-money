...
`Prof-L` and `GP/hr-L` are boxed off with a rule. They are the two figures the decision
rests on — what you make per herb and what that comes to per hour, both at the price you
can count on — and on a row where nearly every number is green, an outline picks them out
where another bright cell could not.
...
it narrows, down to a 26-column floor. Prices keep their deltas at every width. Resizing
